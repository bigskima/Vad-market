...
returns table(public_id uuid,question text,context text,category text,status text,confidence numeric,created_at timestamptz,updated_at timestamptz)
language sql security definer set search_path=''
as $$
  select p.public_id,p.raw_question,p.raw_context,p.normalized_payload->>'submitted_category',p.status,
    nullif(p.normalized_payload->>'proposer_confidence','')::numeric,p.created_at,p.updated_at
...
returns table(proposal_public_id uuid,question text,context text,category text,proposal_status text,proposer_user_id uuid,created_at timestamptz)
...
as $$ begin
  ...
  return query select p.public_id,p.raw_question,p.raw_context,p.normalized_payload->>'submitted_category',p.status,p.proposer_user_id,p.created_at
    from market.proposals p where p.status in ('SUBMITTED','UNDER_REVIEW','NEEDS_CLARIFICATION') order by p.created_at asc;
end; $$;
...
returns table(event_public_id uuid,event_title text,event_status text,resolution_id bigint,resolution_status text,outcome_code text,provisional_at timestamptz,active_disputes bigint)
language plpgsql security definer set search_path=''
as $$ begin
  if not private.has_permission('oracle.review') then raise exception 'Permission required' using errcode='42501'; end if;
  return query select e.public_id,e.title,e.status,r.id,r.status,r.outcome_code,r.created_at,
      (select count(*) from oracle.disputes d where d.event_id=e.id and d.status in ('OPEN','UNDER_REVIEW','ESCALATED'))
    from market.canonical_events e left join oracle.resolutions r on r.event_id=e.id and r.status='PROVISIONAL'
    where e.status in ('AWAITING_ORACLE','PROVISIONALLY_RESOLVED','DISPUTED') order by e.updated_at asc;
end; $$;
...
as $$ declare result jsonb; begin
  if not (private.has_permission('markets.manage') or private.has_permission('finance.read') or private.has_permission('oracle.review')) then
    raise exception 'Permission required' using errcode='42501'; end if;
  select jsonb_build_object(
    ...
    'generatedAt',statement_timestamp()) into result;
  return result;
end; $$;
