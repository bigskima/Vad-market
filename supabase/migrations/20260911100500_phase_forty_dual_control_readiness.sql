-- VAD Phase 40: scoped provider administration + explicit dual-control launch gates.
--
-- Provider activation already enforces maker/checker at the request level. This
-- migration makes that operationally usable without granting a second person full
...
select
  ...
  true
where not exists (
  select 1 from admin.roles where code='PROVIDER_ADMIN'
);

insert into admin.role_permissions(role_id,permission_id)
select r.id,p.id
from admin.roles r
join admin.permissions p on p.code='providers.manage'
...
begin
  ...
  v_gates:=jsonb_build_array(
    ...
    jsonb_build_object('key','ORACLE_INGESTION','category','Operations','title','Oracle evidence ingestion','status',case when v_ingestion_scheduler then 'READY' else 'BLOCKED' end,'detail',case when v_ingestion_scheduler then 'External oracle evidence ingestion is scheduled.' else 'The oracle runtime exists, but automatic external evidence ingestion is not scheduled yet.' end),
