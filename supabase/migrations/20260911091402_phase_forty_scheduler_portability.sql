select cron.schedule(
  ...
  select net.http_post(
    ...
    headers:=jsonb_build_object('Content-Type','application/json','x-vad-scheduler-secret',scheduler_secret.decrypted_secret),
...
begin
  ...
  if not v_is_admin then raise exception 'Admin access required' using errcode='42501'; end if;

  select coalesce(enabled,false) into v_app_ready
  from public.admin_service_posture() where service_key='app_access';

  select exists (
    select 1 from integration.providers p
    ...
    where p.code='DIDIT' and p.environment='PRODUCTION'
      ...
      and r.operation='KYC' and r.status='ACTIVE' and r.country_code='NG'
  ) and coalesce((select enabled from public.admin_service_posture() where service_key='kyc_start'),false)
  into v_kyc_ready;

  select exists (
    select 1 from ai.providers ap
    join integration.providers p on p.id=ap.integration_provider_id
    where p.provider_type='AI' and p.environment='PRODUCTION'
      and p.status in ('ACTIVE','DEGRADED') and ap.status='ACTIVE'
      and coalesce((p.public_metadata->>'configured')::boolean,false)=true
      and ap.capabilities @> '["MARKET_ADMISSION"]'::jsonb
      and coalesce(ap.cost_policy->>'billing_mode','') in ('FREE_ONLY','FREE_TIER','ENABLED')
  ) into v_ai_ready;

  select count(*)::integer into v_active_oracle_policies
  from oracle.policies where status='ACTIVE' and effective_at<=statement_timestamp();

  with launch_pairs(canonical_key) as (
    values ('BTC/USD'::text),('ETH/USD'::text),('USDC/USD'::text)
  ), mapped as (
    select lp.canonical_key,count(distinct r.provider_id)::integer provider_count
    from launch_pairs lp
    left join oracle.provider_resources r
      on r.resource_type='CRYPTO_PAIR' and upper(r.canonical_key)=lp.canonical_key
     and r.status='ACTIVE' and coalesce((r.metadata->>'settlement_eligible')::boolean,false)=true
    left join integration.providers p
      on p.id=r.provider_id and p.provider_type='ORACLE' and p.environment='PRODUCTION'
    ...
    left join integration.providers p
      on p.id=r.provider_id and p.provider_type='ORACLE' and p.environment='PRODUCTION'
     and p.status in ('ACTIVE','DEGRADED') and p.capabilities @> '["CRYPTO_PRICE_THRESHOLD"]'::jsonb
     and coalesce((p.public_metadata->>'configured')::boolean,false)=true
    left join integration.provider_health h on h.provider_id=p.id
    where p.id is not null and h.status in ('HEALTHY','DEGRADED')
    group by lp.canonical_key
  )
  select coalesce((select min(provider_count) from mapped),0),coalesce((select min(provider_count) from ready),0)
  into v_crypto_mapped_quorum,v_crypto_ready_quorum;

  select count(distinct p.id)::integer into v_football_ready
  from integration.providers p
  left join integration.provider_health h on h.provider_id=p.id
  where p.provider_type='ORACLE' and p.environment='PRODUCTION'
    and p.status in ('ACTIVE','DEGRADED') and p.capabilities @> '["FOOTBALL_MATCH_RESULT"]'::jsonb
    and coalesce((p.public_metadata->>'configured')::boolean,false)=true
    and h.status in ('HEALTHY','DEGRADED');

  select exists(select 1 from cron.job where jobname='vad-close-due-markets' and active) into v_close_scheduler;
  select exists(select 1 from cron.job where jobname='vad-evaluate-oracle-consensus' and active) into v_consensus_scheduler;
  select exists(select 1 from vault.decrypted_secrets where name='vad_project_url' and decrypted_secret ~ '^https://[^/]+$') into v_scheduler_endpoint_configured;
  select v_scheduler_endpoint_configured and exists(select 1 from cron.job where jobname='vad-oracle-ingestion' and active) into v_ingestion_scheduler;

  select count(*)::integer into v_money_enabled
  from public.admin_service_posture()
  where service_key in ('trading','deposits','withdrawals','settlement') and enabled;

  select count(distinct p.id)::integer into v_payment_ready
  from integration.providers p join integration.provider_routes r on r.provider_id=p.id
  where p.provider_type='PAYMENT' and p.environment='PRODUCTION'
    and p.status in ('ACTIVE','DEGRADED') and coalesce((p.public_metadata->>'configured')::boolean,false)=true
    and r.status='ACTIVE';

  select count(distinct ur.user_id)::integer into v_provider_operators
  ...
  where perm.code='providers.manage' and ur.revoked_at is null
    and ur.effective_at<=statement_timestamp() and (ur.expires_at is null or ur.expires_at>statement_timestamp());

  select count(distinct ur.user_id)::integer into v_oracle_reviewers
  from admin.user_roles ur
  join admin.role_permissions rp on rp.role_id=ur.role_id
  join admin.permissions perm on perm.id=rp.permission_id
  where perm.code='oracle.review' and ur.revoked_at is null
    and ur.effective_at<=statement_timestamp() and (ur.expires_at is null or ur.expires_at>statement_timestamp());

  v_platform_ready:=v_app_ready and v_kyc_ready and v_ai_ready;
  v_market_lifecycle_ready:=v_platform_ready and v_active_oracle_policies>0 and v_crypto_ready_quorum>=2
    and v_close_scheduler and v_consensus_scheduler and v_ingestion_scheduler
    and v_provider_operators>=2 and v_oracle_reviewers>=2;
  v_real_money_ready:=v_market_lifecycle_ready and v_money_enabled=4 and v_payment_ready>0;
  ...
  select count(*)::integer into v_blockers from jsonb_array_elements(v_gates) item where item->>'status'='BLOCKED';
  select count(*)::integer into v_warnings from jsonb_array_elements(v_gates) item where item->>'status'='WARNING';
  v_release_state:=case when v_real_money_ready then 'LIVE_READY' when v_market_lifecycle_ready then 'MARKET_LIFECYCLE_READY' when v_platform_ready then 'PLATFORM_READY_MARKET_BLOCKED' else 'HARDENING_REQUIRED' end;

  return jsonb_build_object('generatedAt',statement_timestamp(),'releaseState',v_release_state,'platformReady',v_platform_ready,'marketLifecycleReady',v_market_lifecycle_ready,'realMoneyReady',v_real_money_ready,'blockerCount',v_blockers,'warningCount',v_warnings,'gates',v_gates);
end;
$$;
