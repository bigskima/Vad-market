...
begin
  ...
  for v_row in
    select r.id resolution_id,r.event_id,r.outcome_code,r.consensus_evidence,r.created_at provisional_at,
           p.id policy_id,p.public_id policy_public_id,p.consensus_rule,p.dispute_window_seconds,
           e.public_id event_public_id
  ...
  loop
    ...
    -- A policy can intentionally require one deterministic provider. Missing
    -- quorum configuration still defaults to two providers above.
    if v_quorum<1 or v_winning_count<v_quorum then continue; end if;
    if jsonb_typeof(v_row.consensus_evidence->'observations')<>'array' then continue; end if;

    select count(distinct nullif(obs->>'provider_id',''))::integer,
           count(*) filter(where upper(coalesce(obs->>'outcome',''))<>upper(v_row.outcome_code))::integer
    ...
    values(
      'SYSTEM','ORACLE_RESOLUTION_AUTO_FINALIZED','ORACLE_RESOLUTION',v_row.resolution_id::text,
