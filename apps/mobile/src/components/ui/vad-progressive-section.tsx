...
export function VadProgressiveSection({
  ...
  return (
    <VadCard variant={variant} style={{ padding: 0, overflow: 'hidden' }}>
      <Pressable
      ...
      >
        ...
        <View style={{ flex: 1, minWidth: 0, gap: 2 }}>
          {eyebrow ? <VadText variant="caption" tone="brand" numberOfLines={1}>{eyebrow}</VadText> : null}
          <VadText variant="bodyStrong" numberOfLines={2}>{title}</VadText>
          {description ? (
            <VadText variant="caption" tone="secondary" numberOfLines={expanded ? undefined : 2}>{description}</VadText>
          ) : null}
