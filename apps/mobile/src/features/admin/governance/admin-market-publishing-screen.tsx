    ...
    <View style={{ gap: theme.spacing.xl }}>
      ...
      <VadBottomSheet visible={Boolean(pending)} title={actionTitle(pending?.action)} onClose={() => { if (!working) setPending(null); }}>
        {pending ? <View style={{ gap: theme.spacing.md }}>
          <VadCard variant="raised" style={{ gap: 6 }}><VadText variant="bodyStrong">{pending.row.title}</VadText><VadText variant="caption" tone="secondary">{pending.row.category} · {pending.row.asset_code}</VadText><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}><VadChip label={marketFormatLabel(pending.row.market_type)} tone="brand" /><VadChip label={tradingMethodLabel(pending.row.liquidity_model)} tone={pending.row.liquidity_model === 'POOL' ? 'brand' : 'warning'} /></View><VadText variant="caption" tone="tertiary">Closes {new Date(pending.row.closes_at).toLocaleString()}</VadText></VadCard>
...
function marketFormatLabel(value: string) { return value === 'BINARY' ? 'YES / NO' : value.replaceAll('_', ' '); }
function tradingMethodLabel(value: string) { return value === 'POOL' ? 'PEER POOL' : value === 'ORDER_BOOK' ? 'ORDER BOOK' : value.replaceAll('_', ' '); }
