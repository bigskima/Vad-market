...
export function AdminFeeControlsScreen() {
  ...
  const canPropose = hasAdminPermission(data.access, 'fees.propose');

  const [queue, setQueue] = useState<AdminFeeChangeRequest[]>([]);
  ...
  const loadQueue = useCallback(async () => {
    ...
    setQueueError(null);
    try {
      setQueue(await getAdminFeePolicyQueue());
    } catch (error) {
      setQueueError(error instanceof Error ? error.message : 'Fee proposals could not be loaded.');
    } finally {
      setQueueLoading(false);
    }
  }, [canPropose]);
  ...
  if (!data.finance) {
    return (
      <VadErrorState
        ...
        message={data.error ?? data.warning ?? 'The finance policy summary could not be loaded.'}
  ...
  async function submitPolicyChange() {
    ...
    const definition = POLICY_DEFINITIONS.find((item) => item.name === selectedPolicy);
    if (!definition) return;

    const current = policyMap[selectedPolicy];
    ...
    try {
      ...
      setActionError(error instanceof Error ? error.message : 'Enter valid fee percentages.');
      return;
    }
    ...
    try {
      if (isSuperAdmin) {
        ...
        setMessage(`Proposal ${requestId} was submitted for Super Admin approval. It is not active yet.`);
      }

      setSelectedPolicy(null);
      setReason('');
      await Promise.all([data.refresh(), loadQueue()]);
    } catch (error) {
      setActionError(error instanceof Error ? error.message : 'The fee change could not be submitted.');
    } finally {
      setWorking(false);
    }
  }
  ...
  async function decideProposal(decision: 'APPROVE' | 'REJECT') {
    ...
    try {
      ...
      setActionError(error instanceof Error ? error.message : 'The proposal decision could not be completed.');
    } finally {
      setWorking(false);
    }
  }
  ...
  return (
    <View style={{ gap: theme.spacing.xxxl }}>
      <View style={{ gap: theme.spacing.xs }}>
        <VadText variant="label" tone="brand">MONEY · FEE CONTROLS</VadText>
      ...
      <VadCard
        ...
        style={{
          ...
          borderColor: isSuperAdmin ? theme.colors.warning : theme.colors.brandPrimary,
          backgroundColor: isSuperAdmin ? theme.colors.warningSoft : theme.colors.brandSoft,
        }}
      >
        <VadText variant="caption" tone={isSuperAdmin ? 'warning' : 'brand'}>
          {isSuperAdmin ? 'SUPER ADMIN DIRECT AUTHORITY' : 'DUAL-CONTROL GOVERNANCE'}
        </VadText>
      ...
      {message ? (
        <VadCard variant="outlined" style={{ gap: theme.spacing.sm, borderColor: theme.colors.yes }}>
          <VadText variant="caption" tone="yes">FEE GOVERNANCE UPDATED</VadText>
      ...
      <View style={{ gap: theme.spacing.md }}>
        <View style={{ gap: 2 }}>
          ...
          <VadText variant="caption" tone="secondary">
            Percentages below are live policy configuration, not revenue already earned.
          </VadText>
        </View>
      ...
      >
        {queueLoading ? (
          ...
          <VadErrorState title="Fee proposals unavailable" message={queueError} onRetry={() => void loadQueue()} />
        ) : queue.length ? (
          queue.map((request) => (
            <ProposalRow
      ...
      >
        {selectedPolicy ? (
          <View style={{ gap: theme.spacing.lg }}>
            <View style={{ gap: 2 }}>
              <VadText variant="label" tone="brand">{policyLabel(selectedPolicy).toUpperCase()}</VadText>
              <VadText variant="heading">
                {isSuperAdmin ? 'Direct Super Admin activation' : 'Super Admin approval required'}
              </VadText>
            </View>

            {POLICY_DEFINITIONS.find((item) => item.name === selectedPolicy)?.fields.map((field) => (
              <VadInput
                key={field.key}
                label={field.label}
            ...
            <VadInput
              ...
              placeholder={isSuperAdmin ? 'Why should this fee change take effect now?' : 'Why should Super Admin approve this fee change?'}
              multiline
            />

            <VadText variant="caption" tone="tertiary">
              Existing fixed minimum/maximum policy values are preserved by this rate editor. Fee percentages must remain between 0% and 100%.
            </VadText>

            {actionError ? <VadErrorState title="Fee change blocked" message={actionError} /> : null}

            <VadButton
              label={isSuperAdmin ? 'Apply immediately' : 'Submit for Super Admin approval'}
      ...
      >
        {selectedRequest ? (
          <View style={{ gap: theme.spacing.lg }}>
            <View style={{ gap: theme.spacing.xs }}>
              <VadText variant="label" tone="brand">{policyLabel(selectedRequest.policyName).toUpperCase()}</VadText>
              <VadText variant="heading">{proposalConfigurationSummary(selectedRequest)}</VadText>
              <VadText variant="caption" tone="secondary">
                Proposed by {selectedRequest.proposerEmail ?? selectedRequest.proposedBy} · {formatDate(selectedRequest.proposedAt)}
              </VadText>
              <VadText variant="caption" tone="secondary">
            ...
            <VadCard variant="outlined" style={{ gap: 2 }}>
              <VadText variant="caption" tone="warning">STALE-POLICY PROTECTION</VadText>
              <VadText variant="caption" tone="secondary">
                Approval is rejected server-side if the active fee version changed after this proposal was created. This prevents an old proposal from overwriting a newer Super Admin decision.
              </VadText>
            </VadCard>
            ...
            {actionError ? <VadErrorState title="Proposal decision blocked" message={actionError} /> : null}

            <VadButton
              label="Approve & activate"
              loading={working}
              disabled={reviewReason.trim().length < 3 || selectedRequest.proposedBy === data.access.roles.find((role) => role.code === 'SUPER_ADMIN')?.code}
...
function PolicyCard({
  ...
  return (
    <Pressable
      ...
      onPress={onPress}
      style={({ pressed }) => ({ flexGrow: 1, flexBasis: 250, opacity: pressed ? 0.65 : 1 })}
    >
      <VadCard variant="raised" style={{ minHeight: 156, gap: theme.spacing.md }}>
        <View style={{ flex: 1, gap: theme.spacing.xs }}>
          <VadText variant="heading">{definition.label}</VadText>
          <VadText variant="caption" tone="secondary">{definition.description}</VadText>
        </View>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.lg }}>
          {definition.fields.map((field) => (
            <View key={field.key} style={{ minWidth: 90 }}>
              <VadText variant="bodyStrong">{formatPercent(configuration[field.key])}</VadText>
              <VadText variant="caption" tone="tertiary">{field.label.replace(' (%)', '')}</VadText>
            </View>
          ))}
        </View>
  ...
  return (
    ...
    >
      <View style={{ flex: 1, minWidth: 0, gap: 2 }}>
        ...
        <VadText variant="caption" tone="tertiary">
          {request.proposerEmail ? `${request.proposerEmail} · ` : ''}{formatDate(request.proposedAt)}
        </VadText>
      </View>
      <VadText variant="caption" tone="warning">PENDING</VadText>
      {canReview ? <VadText variant="heading" tone="tertiary">›</VadText> : null}
    </Pressable>
  );
}

function policyLabel(name: FeePolicyName) {
  return POLICY_DEFINITIONS.find((item) => item.name === name)?.label ?? name.replaceAll('_', ' ');
}
...
function bpsFromPercent(value: string) {
  ...
  if (!Number.isFinite(numeric) || numeric < 0 || numeric > 100) {
    throw new Error('Fee percentages must be valid numbers between 0% and 100%.');
  }
...
function formatDate(value: string) {
  ...
  return Number.isNaN(date.getTime()) ? 'Pending review' : date.toLocaleString();
}
