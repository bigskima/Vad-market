...
export function SocialConvictionFeed({ markets, canCreatePost, onOpenMarket }: { markets: MarketCatalogItem[]; canCreatePost: boolean; onOpenMarket: (market: MarketCatalogItem) => void }) {
  ...
  async function publish() {
    if (!body.trim()) return;
    setWorking(true);
    try { await publishConvictionPost({ body: body.trim(), postType: market && stance ? 'PREDICTION' : 'ANALYSIS', instrumentPublicId: market?.instrument_public_id ?? null, stanceOutcomeCode: stance }); setBody(''); setMarket(null); setStance(null); setComposerOpen(false); await load(); }
