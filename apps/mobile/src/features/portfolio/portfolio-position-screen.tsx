...
export function PortfolioPositionScreen({ instrumentId, outcomeCode }: { instrumentId: string; outcomeCode: string }) {
  ...
  const portfolioCost = data.positions.reduce((sum, row) => sum + Number(row.total_cost_basis ?? 0), 0);
    ...
    <View style={{ gap: density.sectionGap }}>
      ...
      <View style={{ flexDirection: wide ? 'row' : 'column', gap: theme.spacing.md }}>
        <VadCard style={{ flex: 1.1, borderColor: yes ? theme.colors.yes : theme.colors.no, gap: theme.spacing.sm }}>
          <VadText variant="caption" tone={yes ? 'yes' : 'no'}>RECORDED COST BASIS</VadText>
          <VadText variant="display" numberOfLines={1} adjustsFontSizeToFit>{money(costBasis)}</VadText>
        ...
        <VadCard variant="raised" style={{ flex: 0.9, gap: theme.spacing.sm }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: theme.spacing.sm }}>
            <View style={{ flex: 1, gap: 1 }}>
              <VadText variant="caption" tone="secondary">PORTFOLIO COST WEIGHT</VadText>
              <VadText variant="heading">{pct(portfolioWeight)}</VadText>
            </View>
            <VadText variant="caption" tone="tertiary">of recorded cost</VadText>
          </View>
          ...
          <VadText variant="caption" tone="secondary">Concentration across the position rows currently returned by your portfolio.</VadText>
        </VadCard>
      </View>
      ...
      <View style={{ flexDirection: wide ? 'row' : 'column', alignItems: 'flex-start', gap: theme.spacing.md }}>
        ...
        <VadCard variant="raised" style={{ flex: 0.9, width: '100%', gap: theme.spacing.sm }}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.xs }}>
            <ContextFact label="Portfolio cost" value={money(portfolioCost)} />
            <ContextFact label="This position" value={money(costBasis)} />
