...
export function TradingTicket({
  ...
  async function execute() {
    ...
    try {
      ...
      Alert.alert(
        'Order placed',
        'Your order is live and may fill when compatible liquidity exists.',
      );
      setQuote(null);
      await onPlaced();
    } catch (error) {
      ...
      >
        ...
        >
          <View style={{ flex: 1 }}>
            <VadInput
              label="Limit price"
              hint="Enter a probability price above 0 and up to 1."
              value={price}
              onChangeText={(value) => {
                setPrice(value);
                setQuote(null);
          ...
          <View style={{ flex: 1 }}>
            <VadInput
              label="Shares"
              hint="Choose a preset below or enter your own quantity."
              value={quantity}
              onChangeText={(value) => {
                setQuantity(value);
                setQuote(null);
        ...
        {!canTrade ? (
          ...
          >
            <VadText variant="caption" tone="warning">
              Trading is not available for this account under the current
              platform policy.
            </VadText>
          </View>
        ) : null}
