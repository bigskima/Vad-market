...
export async function getMyProfile() {
  ...
  if (error) throw userFacingError(error, 'profile', 'We could not load your profile right now. Please try again.');
...
export async function updateMyProfile(input: { displayName: string; handle: string; bio: string }) {
  ...
  if (error) {
    ...
    throw userFacingError(error, 'profile');
  }
}
...
export async function uploadProfileMedia(kind: 'avatar' | 'banner', bytes: ArrayBuffer, mimeType: string) {
  ...
  if (uploadError) throw userFacingError(uploadError, 'profile', 'We could not upload that image right now. Please try again.');
  ...
  if (profileError) throw userFacingError(profileError, 'profile');
  return path;
}
...
export async function getCreatorPublicProfile(creatorUserId: string) {
  ...
  if (error) throw userFacingError(error, 'social', 'We could not load this creator profile right now. Please try again.');
  if (!data) return null;
  return {
